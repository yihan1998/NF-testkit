...
ipsec_security_gw_init_doca_flow(const struct ipsec_security_gw_config *app_cfg, int nb_queues, struct ipsec_security_gw_ports_map *ports[])
{
	...
	flow_cfg.nr_shared_resources[DOCA_FLOW_SHARED_RESOURCE_CRYPTO] = MAX_NB_RULES * 2;
		...
		enum ipsec_security_gw_esp_offload offload, enum doca_ipsec_direction direction)
{
	...
	match_mask.meta.pkt_meta = (1 << 31) | (1 << 30);
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to add entry to RSS pipe: %s", doca_get_error_string(result));
		return result;
	}

	match.meta.pkt_meta = 1 << 31;
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to process entry: %s", doca_get_error_string(result));
		return result;
	}
...
create_empty_pipe(struct doca_flow_pipe **pipe)
{
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to create empty pipe: %s", doca_get_error_string(result));
		return result;
	}
	return DOCA_SUCCESS;
}
...
create_switch_port_meta_pipe(struct doca_flow_pipe **pipe)
{
	...
	result = doca_flow_pipe_create(&pipe_cfg, NULL, NULL, pipe);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to create switch port meta pipe: %s", doca_get_error_string(result));
		return result;
	}
			     ...
			     struct doca_flow_pipe *decrypt_root, struct doca_flow_pipe *pipe)
{
	...
	match.meta.port_meta = ports[UNSECURED_IDX]->port_id;
	...
	match.meta.port_meta = ports[SECURED_IDX]->port_id;
...
create_switch_pkt_meta_pipe(struct doca_flow_pipe **pipe)
{
	...
	result = doca_flow_pipe_create(&pipe_cfg, NULL, NULL, pipe);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to create pkt meta pipe: %s", doca_get_error_string(result));
		return result;
	}
	return DOCA_SUCCESS;
}
...
add_switch_pkt_meta_entries(struct ipsec_security_gw_ports_map *ports[], struct doca_flow_pipe *encrypt_pipe, struct doca_flow_pipe *match_port_pipe, struct doca_flow_pipe *pipe)
{
	...
	result = doca_flow_pipe_add_entry(0, pipe, &match, NULL, NULL, &fwd, DOCA_FLOW_WAIT_FOR_BATCH, &status, NULL);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to add entry to pkt meta pipe: %s", doca_get_error_string(result));
		return result;
	}
	...
	match.meta.pkt_meta = 1 << 31; /* first bit is one */
	fwd.type = DOCA_FLOW_FWD_PORT;
			...
			struct doca_flow_pipe *decrypt_root, struct doca_flow_pipe *encrypt_pipe)
{
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to create port meta pipe: %s", doca_get_error_string(result));
		return result;
	}

	result = add_switch_port_meta_entries(ports, encrypt_root, decrypt_root, match_port_pipe);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to add port meta pipe entries: %s", doca_get_error_string(result));
		return result;
	}
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to add pkt meta pipe entries: %s", doca_get_error_string(result));
		return result;
	}
