...
extern flexio_func_t ddos_device_init;
extern flexio_func_t ddos_device_event_handler;

void ddos_steering_rules_destroy(struct app_config *app_cfg);
void ddos_ibv_device_destroy(struct app_config *app_cfg);
void ddos_destroy(struct app_config *app_cfg);
void ddos_device_resources_destroy(struct app_config *app_cfg);
void ddos_device_destroy(struct app_config *app_cfg);
...
int setup_device(struct app_config *app_cfg)
{
	...
	event_handler_attr.host_stub_func = ddos_device_event_handler;
...
int allocate_device_resources(struct app_config *app_cfg)
{
	...
	for (int i = 0; i < app_cfg->nb_dpa_threads; i++) {
		...
		ctx->dev_data->haddr = (uint64_t)app_cfg->host_buffer;
...
int ddos_run_device_process(struct app_config *app_cfg)
{
	int ret = 0;
...
int ddos_create_steering_rule_rx(struct app_config *app_cfg)
{
	int result;
...
int create_steering_rule_tx(struct app_config *app_cfg, struct dpa_process_context * ctx)
{
	...
	DEVX_SET(dr_match_spec, match_mask->match_buf, udp_dport, ctx->udp_port);
...
int ddos_create_steering_rule_tx(struct app_config *app_cfg)
{
	int result;
...
int ddos_run_event_handler(struct app_config *app_cfg) {
	int ret = 0;
	for (int i = 0; i < app_cfg->nb_dpa_threads; i++) {
		struct dpa_process_context * ctx = app_cfg->context[i];
...
void ddos_destroy(struct app_config *app_cfg)
{
	...
	ddos_steering_rules_destroy(app_cfg);

	// /* Destroy WQs */
	ddos_device_resources_destroy(app_cfg);

	/* Destroy FlexIO resources */
	ddos_device_destroy(app_cfg);

	/* Destroy DevX and IBV resources */
	ddos_ibv_device_destroy(app_cfg);
...
int main(int argc, char ** argv)
{
	...
	result = ddos_run_device_process(&app_cfg);
	if (result < 0) {
		printf("Failed to call init function on device\n");
	...
	result = ddos_create_steering_rule_rx(&app_cfg);
	if (result < 0) {
		printf("Failed to create RX steering rule\n");
		goto device_resources_cleanup;
	}
	...
	result = ddos_create_steering_rule_tx(&app_cfg);
	if (result < 0) {
