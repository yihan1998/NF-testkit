...
uint64_t echo_device_init(uint64_t data);
flexio_dev_event_handler_t echo_device_event_handler; /* Event handler function */

uint16_t htons(uint16_t hostshort);
uint32_t htonl(uint32_t hostlong);
uint32_t echo_encap(char *out_data, char *in_data, uint32_t in_data_size);

__dpa_rpc__ uint64_t echo_device_init(uint64_t data)
{
...
void __dpa_global__ echo_device_event_handler(uint64_t index)
{
