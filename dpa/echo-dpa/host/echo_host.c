...
extern flexio_func_t echo_device_init;
extern flexio_func_t echo_device_event_handler;

void echo_steering_rules_destroy(struct app_config *app_cfg);
void echo_ibv_device_destroy(struct app_config *app_cfg);
void echo_destroy(struct app_config *app_cfg);
void echo_device_resources_destroy(struct app_config *app_cfg);
void echo_device_destroy(struct app_config *app_cfg);
...
int setup_device(struct app_config *app_cfg)
{
	...
	event_handler_attr.host_stub_func = echo_device_event_handler;
...
int echo_run_device_process(struct app_config *app_cfg)
{
	int ret = 0;
...
int echo_create_steering_rule_rx(struct app_config *app_cfg)
{
	int result;
...
int echo_create_steering_rule_tx(struct app_config *app_cfg)
{
	int result;
...
int echo_run_event_handler(struct app_config *app_cfg) {
	int ret = 0;
	for (int i = 0; i < app_cfg->nb_dpa_threads; i++) {
		struct dpa_process_context * ctx = app_cfg->context[i];
...
void echo_destroy(struct app_config *app_cfg)
{
	...
	echo_steering_rules_destroy(app_cfg);

	// /* Destroy WQs */
	echo_device_resources_destroy(app_cfg);

	/* Destroy FlexIO resources */
	echo_device_destroy(app_cfg);

	/* Destroy DevX and IBV resources */
	echo_ibv_device_destroy(app_cfg);
...
int main(int argc, char ** argv)
{
	...
	result = echo_run_device_process(&app_cfg);
	if (result < 0) {
		printf("Failed to call init function on device\n");
	...
	result = echo_create_steering_rule_rx(&app_cfg);
	if (result < 0) {
		printf("Failed to create RX steering rule\n");
		goto device_resources_cleanup;
	}
	...
	result = echo_create_steering_rule_tx(&app_cfg);
	if (result < 0) {
		printf("Failed to create TX steering rule\n");
		goto rule_cleanup;
	}

	printf("Run ECHO handler...\n");
	result = echo_run_event_handler(&app_cfg);
	if (result < 0) {
		printf("Failed to run event handler on device\n");
		goto rule_cleanup;
	}
