...
extern flexio_func_t flow_monitor_device_init;
extern flexio_func_t flow_monitor_device_event_handler;

void flow_monitor_steering_rules_destroy(struct app_config *app_cfg);
void flow_monitor_ibv_device_destroy(struct app_config *app_cfg);
void flow_monitor_destroy(struct app_config *app_cfg);
void flow_monitor_device_resources_destroy(struct app_config *app_cfg);
void flow_monitor_device_destroy(struct app_config *app_cfg);
...
int flow_monitor_run_device_process(struct app_config *app_cfg)
{
	int ret = 0;
...
int flow_monitor_create_steering_rule_rx(struct app_config *app_cfg)
{
	int result;
...
int flow_monitor_create_steering_rule_tx(struct app_config *app_cfg)
{
	int result;
...
int flow_monitor_run_event_handler(struct app_config *app_cfg) {
	int ret = 0;
	for (int i = 0; i < app_cfg->nb_dpa_threads; i++) {
		struct dpa_process_context * ctx = app_cfg->context[i];
...
void flow_monitor_destroy(struct app_config *app_cfg)
{
	...
	flow_monitor_steering_rules_destroy(app_cfg);

	// /* Destroy WQs */
	flow_monitor_device_resources_destroy(app_cfg);

	/* Destroy FlexIO resources */
	flow_monitor_device_destroy(app_cfg);

	/* Destroy DevX and IBV resources */
	flow_monitor_ibv_device_destroy(app_cfg);
...
int main(int argc, char ** argv)
{
	...
	result = flow_monitor_run_device_process(&app_cfg);
	if (result < 0) {
		printf("Failed to call init function on device\n");
	...
	result = flow_monitor_create_steering_rule_rx(&app_cfg);
	if (result < 0) {
		printf("Failed to create RX steering rule\n");
		goto device_resources_cleanup;
	}
	...
	result = flow_monitor_create_steering_rule_tx(&app_cfg);
	if (result < 0) {
		printf("Failed to create TX steering rule\n");
		goto rule_cleanup;
	}

	printf("Run FLOW_MONITOR handler...\n");
	result = flow_monitor_run_event_handler(&app_cfg);
	if (result < 0) {
		printf("Failed to run event handler on device\n");
		goto rule_cleanup;
	}
